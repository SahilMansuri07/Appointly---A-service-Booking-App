...
const bypassRoutes = [
    "/api/v1/auth/login",
     "/api/v1/auth/sign-up",
    "/api/v1/auth/logout",
    "/api/v1/user/service-list",
    "/api/v1/user/get-available-slots",
    "/api/v1/user/book-appointment",
    "/api/v1/user/service-details",
]
...
async function tokenMiddleware(req, res, next) {
    ...
    //    console.log(token)
        ...
        if (device[0] && device[0][0] && device[0][0].id) {
            const user = await common.getUserDetails({ id: device[0][0].user_id })
            // console.log(user)
